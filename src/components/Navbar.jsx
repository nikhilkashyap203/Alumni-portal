...
const Navbar = () => {
  ...
  const linkClasses = ({ isActive }) =>
    `text-white hover:text-gray-200 font-medium transition ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <div className="flex-shrink-0 flex items-center">
            <HeaderLogo />
            <span className="text-white text-lg sm:text-xl font-bold ml-2">
              Alumni Portal
            </span>
          </div>
        ...
        <div className={`${isMenuOpen ? "block" : "hidden"} md:hidden`}>
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            <NavLink to="/college" className={linkClasses} onClick={() => setIsMenuOpen(false)}>
              College
            </NavLink>
            <NavLink to="/students" className={linkClasses} onClick={() => setIsMenuOpen(false)}>
              Students
            </NavLink>
            <NavLink to="/user" className={linkClasses} onClick={() => setIsMenuOpen(false)}>
              User
            </NavLink>
          </div>
        </div>
